...
ofMesh convertIndices(ofMesh& mesh) {
    ofMesh result;
    int n = mesh.getNumIndices();
    for(int i = 0; i < n; i++) {
        int j = mesh.getIndex(i);
        result.addVertex(mesh.getVertex(j));
        if(mesh.hasColors()) {
            result.addColor(mesh.getColor(j));
        }
        if(mesh.hasTexCoords()) {
            result.addTexCoord(mesh.getTexCoord(j));
        }
        if(mesh.hasNormals()) {
            result.addNormal(mesh.getNormal(j));
        }
    }
    return result;
}